...
export class AuthService {
  ...
  async changePassword(id: string, passwordNew: string, adminUser?: any) {
    const user = await this.userRepository.findById(id);
    if (!user) throw new Error('Usuario no encontrado');
  ...
  async createRole(data: any, adminUser?: any) {
    if (!this.roleRepository) throw new Error('Repositorio de roles no configurado');
  ...
  async updateRole(id: string, data: any, adminUser?: any) {
    if (!this.roleRepository) throw new Error('Repositorio de roles no configurado');
