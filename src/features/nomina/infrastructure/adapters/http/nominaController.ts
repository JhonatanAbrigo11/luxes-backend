...
export class NominaController {
  ...
  async getOvertime(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async saveOvertimeBulk(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async getPayrolls(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const ingresosByEmpleado = new Map<string, typeof ingresosBatch>();
      ...
      const egresosByEmpleado = new Map<string, typeof egresosBatch>();
      ...
      const userIndexInTransaction = new Map<string, number>();
      ...
      for (const emp of empleados) {
        ...
        for (const e of empEgresos) {
          ...
          if (e.tipo === 'ANTICIPO') anticiposSum += mVal;
          else if (e.tipo === 'MULTA') multasSum += mVal;
          else if (e.tipo === 'OTROS') otrosSum += mVal;
        }
        ...
        if (!existing) {
          ...
          if (existing.estado === "PENDIENTE" || existing.estado === "ABONO_PARCIAL") {
            const currentIngresos = (existing.ingresos as any) || {};
            const currentEgresos = (existing.egresos as any) || {};
  ...
  async savePayroll(req: Request, res: Response): Promise<Response> {
    try {
      const data = req.body;
  ...
  async getVacaciones(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async saveVacacion(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async createDetailedEgreso(req: Request, res: Response): Promise<Response> {
    try {
  ...
  async deleteDetailedEgreso(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      if (!id) {
        return res.status(400).json({
  ...
  private async recalculatePayrollForEgreso(empleadoId: string, fechaEgreso: Date): Promise<void> {
    ...
    if (payroll) {
      ...
      const currentEgresos = (payroll.egresos as any) || {};
  ...
  async getDetailedIngresos(req: Request, res: Response): Promise<Response> {
    try {
      const { empleadoId, fechaInicio, fechaFin } = req.query;
  ...
  async createDetailedIngreso(req: Request, res: Response): Promise<Response> {
    try {
      const { empleadoId, tipo, monto, fecha, motivo } = req.body;
      if (!empleadoId || !tipo || monto === undefined || !fecha) {
        return res.status(400).json({
  ...
  async deleteDetailedIngreso(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      if (!id) {
        return res.status(400).json({
  ...
  async exportToExcel(req: Request, res: Response): Promise<void> {
    try {
      ...
      const mapNominaRow = (emp: any, rawNomina: any, quincenaNum: number, fInicio: Date, fFin: Date) => {
        ...
        const totalAbonado = rawNomina?.abonos ? rawNomina.abonos.reduce((sum: number, ab: any) => sum + Number(ab.monto), 0) : 0;
      ...
      const addQuincenaSheet = (sheetName: string, titleLabel: string, nominasList: any[], quincenaNum: number, fInicio: Date, fFin: Date) => {
        ...
        const applyHeaderStyles = (rowNum: number, bgColor: string, fgColor: string) => {
        ...
        dataRows.forEach((rowValues) => {
          ...
          row.eachCell((cell, colNum) => {
            ...
            else if (colNum === 3 || colNum === 18) cell.alignment = { horizontal: 'center' };
            else if (colNum === 5) cell.alignment = { horizontal: 'center' };
        ...
        sheet.columns.forEach((column, index) => {
          ...
          if (colIndex === 1) column.width = 6;
          else if (colIndex === 2) column.width = 28;
          else if (colIndex === 3) column.width = 12;
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="Nomina_${mesName}_${yVal}.xlsx"`
      );
