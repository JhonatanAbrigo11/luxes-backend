...
export class PrismaTareasAdapter implements TareasRepositoryPort {
  ...
  async findAll(options?: {
    ...
    if (options?.prioridad) where.prioridad = options.prioridad;
    if (options?.search) {
      where.OR = [
    ...
    return { items: items as unknown as TareaData[], total };
  }
  ...
  async findById(id: string): Promise<TareaData | null> {
    const row = await this.prisma.tarea.findUnique({
      where: { id },
      include: tareaInclude,
    });
    return row as unknown as TareaData | null;
  }
  ...
  async create(data: {
    ...
    let parsedFechaLimite: Date | null = null;
    if (data.fechaLimite) {
    ...
    return row as unknown as TareaData;
  }
  ...
  async update(id: string, data: {
    ...
    if (!currentTarea) throw new Error('Tarea no encontrada.');
